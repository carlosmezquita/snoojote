...
export const execute = async (interaction: ChatInputCommandInteraction, client: DiscordBot) => {
    ...
    try {
        await ticketMemberService.removeMember(channel, user);
        await interaction.reply({
            embeds: [createSuccessEmbed('User Removed', `Successfully removed ${user.toString()} from the ticket.`)]
        });
    } catch (error) {
        console.error('Error removing user from ticket:', error);
        await interaction.reply({
            embeds: [createErrorEmbed('Error', 'Failed to remove user from the ticket.')],
            ephemeral: true
        });
    }
};
