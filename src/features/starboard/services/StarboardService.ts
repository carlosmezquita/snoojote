import { Message, TextChannel, EmbedBuilder, Events, PartialMessage } from 'discord.js';
...
export class StarboardService {
    public static async handleReactionUpdate(message: Message | PartialMessage) {
        ...
        // DB Check
        const [existing] = await db.select().from(starboardMessages).where(eq(starboardMessages.originalMessageId, fullMessage.id));

        const starboardChannel = fullMessage.client.channels.cache.get(starboardChannelId) as TextChannel;
        if (!starboardChannel) {
    ...
    private static createStarboardEmbed(message: Message, emoji: string, count: number): EmbedBuilder {
